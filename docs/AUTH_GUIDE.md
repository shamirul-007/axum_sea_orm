...
This (re)creates `src/entities/user.rs`, `refresh_token.rs`, `sea_orm_active_enums.rs`, `mod.rs`,
and the relation wiring — all matching the schema you just migrated. It's exactly how the existing
entities in this repo were made (hence the `@generated by sea-orm-codegen` headers).
...
> ⚠️ **Generation can't know about three of our hand-tweaks.** After generating (whole-folder or
...
> 2. The `impl Role { fn as_str(...) }` helper in `sea_orm_active_enums.rs` — our own code uses it.
> 3. Double-check the `Uuid` primary keys came out as `#[sea_orm(primary_key, auto_increment = false)]`.
>
...
3. Entities: generate with `sea-orm-cli generate entity`, then re-apply the 3 tweaks (Step 2)
...
src/entities/{user.rs (rewrite), refresh_token.rs (new),
              sea_orm_active_enums.rs (new), mod.rs (register)}
